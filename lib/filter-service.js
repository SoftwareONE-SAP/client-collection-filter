FilterService = function(model) {
  ...
  self._startDate = new ReactiveVar();
  self._endDate = new ReactiveVar();
  ...
  self._generateFilterHandle = Tracker.autorun(function() {
    ...
    if (!_.isNull(dateFilter) && dateFilter.enabled) {
      var startDate = self._startDate.get(),
        endDate = self._endDate.get();
...
FilterService.prototype.setDateRange = function(startDate, endDate) {
  ...
  if (_.isDate(startDate)) {
    self._startDate.set(startDate);
  }
  if (_.isDate(endDate)) {
    self._endDate.set(endDate);
  }
};
