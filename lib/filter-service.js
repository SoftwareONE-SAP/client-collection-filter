FilterService = function(model) {
  ...
  /**
   * Allowed field Types
   * `enabled` - True to allow filter type
   * `fn` - function for transferring data to an existing field
   *        function(field, existingField) {
   *          // Code to modify field object
   *          // does not need to return field; as passed by reference
   *        }
   *
   * Note: Default allowed types are:
   * - date
   * - enum
   * - number
   * - range
  ...
  self._allowedTypes = {
    'date': {
      enabled: true,
      fn: function(field, existingField) {
        var self = this;
    ...
    'enum': {
      ...
      fn: function(field, existingField) {
        ...
        field.options.default = _.has(field.options, 'default') ? field.options.default : true;
        ...
        _.each(field.options.data, function(option, key) {
          ...
          if (!_.has(option, 'enabled')) {
            option.enabled = field.options.default;
          }
...
FilterService.prototype._generateField = function(field, existingField) {
  ...
  if (!_.has(self._allowedTypes, obj.type) || !self._allowedTypes[obj.type].enabled) {
    throw new Meteor.Error(
...
FilterService.prototype._processFieldData = function(field, existingField) {
  ...
  /**
   * Error if unknown field type
   */
  if (!_.has(self._allowedTypes, field.type) || !self._allowedTypes[field.type].enabled) {
    throw new Meteor.Error(
      'FilterService-field-type',
  ...
  /**
   * Process field type specific data
   */
  self._allowedTypes[field.type].fn.call(self, field, existingField);
...
FilterService.prototype._generateFilters = function() {
   ...
   */
  _.each(fields, function(field) {
    if (_.last(self._allowedTypes, field.type) &&
      field.enabled &&
      _.indexOf(['string', 'number'], field.type) === -1
    ) {
...
/**
 * Registers the custom type of field with the filter service
 * @param {String} type Type name
 */
FilterService.prototype.addFieldType = function(type, processFunction) {
  var self = this;
  if (_.has(self._allowedTypes, type)) {
    throw new Meteor.Error(
      'FilterServer-addFieldType',
      'Field Type `' + type + '` already exists'
    );
  }
  self._allowedTypes[type] = {
    enabled: true,
    fn: processFunction || function(field, existingField) {
      //No operation
    }
  };
};
...
/**
 * Enables the field type
 * @param  {String} type Type name
 */
FilterService.prototype.enableFieldType = function(type) {
  var self = this;
  if (!_.has(self._allowedTypes, type)) {
    throw new Meteor.Error(
      'FilterServer-enableFieldType',
      'Unknown Field Type: `' + type + '`'
    );
  }
  self._allowedTypes[type].enabled = true;
};

/**
 * Disables the field type
 * @param  {String} type Type name
 */
FilterService.prototype.disableFieldType = function(type) {
  var self = this;
  if (!_.has(self._allowedTypes, type)) {
    throw new Meteor.Error(
      'FilterServer-enableFieldType',
  ...
  self._allowedTypes[type].enabled = false;
};
...
/**
 * Disables all fields and clears the text filter
 */
FilterService.prototype.disableAllFields = function() {
  var self = this;
  self.setTextFilter('');
  _.chain(self.fields())
    ...
    .each(function(field) {
      self.setFieldEnabled(field.key, false);
      if (field.type === 'enum' && field.options.resetOnDisable) {
        _.chain(field.options.data)
          .filter(function(opt) {
            return opt.enabled !== field.options.default;
          })
          .each(function(opt) {
            self.setEnumOption(
              field.key,
...
/**
 * Set the `enabled` value for all of the options in the field
 * @param {String}  fieldKey the field's `key`
 * @param {Boolean} value    The value to change all options to
 */
FilterService.prototype.setAllEnumOptions = function(fieldKey, value) {
  var self = this,
    field = self.field(fieldKey);
  _.each(field.options.data, function(datum) {
    datum.enabled = value
  });
  return self._setField(field);
};
