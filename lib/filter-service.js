...
FilterService.prototype._processFieldData = function(field, existingField) {
  var self = this,
    requiresData = function(field) {
      ...
      if (!_.isObject(field.options.data) || _.isArray(field.options.data)) {
        throw new Meteor.Error(
          'FilterService-object-data',
  ...
  switch (field.type) {
    ...
    case 'date':
      ...
      _.each(['start', 'end'], function(key) {
        var originalValue = field.options.data[key];
        switch (typeof field.options.data[key]) {
          case 'object':
            if (field.options.data[key] instanceof Date) {
              break;
            }
          case 'string':
          case 'number':
            field.options.data[key] = new Date(field.options.data[key]);
            break;
          default:
            field.options.data[key] = new Date();
            break;
        }

        if (_.isNaN(field.options.data[key].getTime())) {
          throw new Meteor.Error(
            'FilterService-object-data',
         ...
         */
        if (_.has(existingField, 'options') && _.has(existingField.options, 'data') && _.has(existingField.options.data[key])) {
          field.options.data[key] = existingField.options.data[key];
        }
...
FilterService.prototype._defaultFn = {
  'date': function(field) {
    ...
    switch (field.options.type) {
      case 'before':
        filter[field.key] = {
          $lte: moment(field.options.data.start).endOf('day').toDate()
        };
        break;
      case 'after':
        filter[field.key] = {
          $gte: moment(field.options.data.start).startOf('day').toDate()
        };
        break;
      case 'between':
        filter[field.key] = {
          $gte: moment(field.options.data.start).startOf('day').toDate(),
          $lte: moment(field.options.data.end).endOf('day').toDate()
        };
        break;
      case 'equals':
        filter[field.key] = {
...
FilterService.prototype.setEnumOption = function(fieldKey, option, value) {
  ...
  field.options.data[option].enabled = value;
  return self._setField(field);
};
...
FilterService.prototype.setDateValues = function(fieldKey, startDate, endDate) {
  var self = this,
    field = self.field(fieldKey);
  field.options.data.start = startDate;
  field.options.data.end = endDate;
  return self._setField(field);
};
